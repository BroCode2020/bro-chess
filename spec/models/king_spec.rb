...
RSpec.describe King, type: :model do
  ...
  describe 'movement of the king' do
    ...
    it 'should return false if piece of the same color is in target position' do
      ...
      target_piece = FactoryBot.create(:rook, color: 0, game: game_b, x_pos: 4, y_pos: 5)
      expect(king_b.valid_move?(4, 5)).to eq false
    end

    it 'should return true if king is casling' do
      game_b = FactoryBot.create(:game)
      game_b.pieces.clear
      king_b = FactoryBot.create(:king, color: 0, game: game_b, x_pos: 4, y_pos: 0)
      target_piece = FactoryBot.create(:rook, color: 0, game: game_b, x_pos: 0, y_pos: 0)
      expect(king_b.valid_move?(2, 0)).to eq true
     end
 end
end
    ...
    context 'right rook has moved' do
      ...
      it 'returns true to left rook' do
        ...
        context 'piece is between rook and castle' do
          ...
          #   it 'returns false' do
          #     FactoryBot.create(:queen, game: game_c, x_pos: 4, y_pos: 7, color: 0)
          #     expect(king_cw.castle?(7, 0)).to eq false
          #   end
          # end
          # context 'king moves through space in check' do
          #   it 'returns false' do
          #     FactoryBot.create(:queen, game: game_c, x_pos: 5, y_pos: 7, color: 0)
          #     expect(king_cw.castle?(7, 0)).to eq false
          #   end
          # end
          # context 'castling results in check' do
          #   it 'returns false' do
          #     FactoryBot.create(:queen, game: game_c, x_pos: 6, y_pos: 7, color: 0)
          #     expect(king_cw.castle?(7, 0)).to eq false
          #   end
          # end
        end
